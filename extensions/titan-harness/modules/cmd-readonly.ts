 ...
 *
 * /titan-opinion: every configured slot answers independently.
 * /titan-debate:  N-way all-to-all debate — every round each survivor receives every
...
export function registerReadonlyCommands(pi: ExtensionAPI, h: HarnessDeps): void {
	// ── /titan-opinion — N independent read-only opinions ─────
	pi.registerCommand("titan-opinion", {
		...
		handler: async (raw, ctx) => {
			...
			if (!prompt) {
				ctx.ui.notify("Usage: /titan-opinion <prompt>", "warning");
				return;
			}
			...
			h.panel({ kind: "prompt", command: "titan-opinion", ok: true }, `/titan-opinion ${prompt}`);
			const stopper = h.startStoppable(ctx, "titan-opinion");
			const stopWidget = h.startGridWidget(ctx, "titan-opinion", runs, undefined, startedAt);
			...
			try {
				...
				if (stopper.stopped()) {
					h.stoppedPanel("titan-opinion", runs, artifactsDir, startedAt, "All active opinions were stopped; completed answers remain on disk.");
					return;
				}
				const ok = runs.every(runOk);
				h.panel({ kind: "multi", command: "titan-opinion", title: "◆ OPINIONS — ALL CONFIGURED AGENTS", ok, prompt, sources: runs.map(toStat), answers: runs.map((run) => ({ role: run.role, model: run.model, text: runOk(run) ? run.text : `FAILED: ${runError(run)}`, slotId: run.slot!.id, slotName: run.slot!.name, color: run.slot!.color, primary: run.slot!.primary })), artifactsDir, ...h.totals(runs, startedAt) }, runs.map((run) => `## ${run.slot!.name} · ${run.model}\n${runOk(run) ? run.text : `FAILED: ${runError(run)}`}`).join("\n\n"));
				await h.save(artifactsDir, "summary.json", JSON.stringify({ command: "titan-opinion", ok, agents: runs.map(toStat), sessions: Object.fromEntries(slots.map((slot) => [slot.id, runs.find((run) => run.slot?.id === slot.id)?.sessionRef ?? h.cachedSlotId(slot)])), ...h.totals(runs, startedAt) }, null, 2));
			} finally {
				await h.ensureSummary(artifactsDir, { command: "titan-opinion", ok: false, stopped: stopper.stopped(), agents: runs.map(toStat), sessions: Object.fromEntries(slots.map((slot) => [slot.id, runs.find((run) => run.slot?.id === slot.id)?.sessionRef ?? h.cachedSlotId(slot)])), ...h.totals(runs, startedAt) });
				stopper.release();
				stopWidget();
	...
	// ── /titan-debate — N-way concrete opinions, all-to-all between rounds ──
	pi.registerCommand("titan-debate", {
		...
		handler: async (raw, ctx) => {
			...
			if (rounds < 2) {
				ctx.ui.notify("titan-harness: /titan-debate requires at least 2 rounds (opening + closing)", "error");
				return;
			}
			if (!prompt) {
				ctx.ui.notify("Usage: /titan-debate [--rounds N] <prompt>", "warning");
				return;
			}
			const stack = h.modelStack();
			const slots = orderedSlots(stack);
			...
			await h.save(artifactsDir, "prompt.md", prompt);
			await h.save(artifactsDir, "stack.json", JSON.stringify(stack, null, 2));
			h.panel({ kind: "prompt", command: "titan-debate", ok: true }, `/titan-debate ${(raw ?? "").trim()}`);
			h.panel({ kind: "banner", command: "titan-debate", ok: true, prompt, roles: slots.map((slot) => ({ role: (slot.architect ? "ARCHITECT" : "BUILDER") as AgentRun["role"], model: slot.model, slotId: slot.id, slotName: slot.name, color: slot.color, primary: slot.primary, architect: slot.architect })), maxRounds: rounds, artifactsDir }, "");
			const stopper = h.startStoppable(ctx, "titan-debate");
			const stopWidget = h.startGridWidget(ctx, "titan-debate", runs, undefined, startedAt);
			let previous: AgentRun[] = [];
			try {
				for (let round = 1; round <= rounds; round++) {
					const active = round === 1 ? runs : runs.filter((run) => runOk(run));
					if (active.length < 2) {
						h.panel({ kind: "error", command: "titan-debate", ok: false, round, maxRounds: rounds, sources: runs.map(toStat), artifactsDir, ...h.totals(runs, startedAt) }, `N-way debate halted: fewer than two surviving opinions remain before round ${round}.`);
						return;
					}
					...
					try {
						...
						h.panel({ kind: "error", command: "titan-debate", ok: false, round, maxRounds: rounds, sources: runs.map(toStat), artifactsDir }, `Debate could not build a complete all-opinions packet: ${error instanceof Error ? error.message : String(error)}. Full prior statements remain in ${artifactsDir}/debate/.`);
						return;
					}
					...
					if (stopper.stopped()) {
						h.stoppedPanel("titan-debate", runs, artifactsDir, startedAt, `Stopped during round ${round}; completed opinions remain on disk.`);
						return;
					}
					if (round < rounds) {
						const missingSessions = runs.filter((run) => runOk(run) && !run.sessionRef);
						if (missingSessions.length) {
							h.panel({ kind: "error", command: "titan-debate", ok: false, round, maxRounds: rounds, sources: runs.map(toStat), artifactsDir }, `Debate cannot preserve round history: no resumable session was reported for ${missingSessions.map((run) => run.slot!.name).join(", ")}.`);
							return;
						}
					}
					previous = runs.map((run) => ({ ...run, flow: [...run.flow] }));
					if (round < rounds) {
						h.panel({ kind: "multi", command: "titan-debate", title: `⚔ ROUND ${round}/${rounds} — ${round === 1 ? "OPENING OPINIONS" : "UPDATED SIDES"}`, ok: runs.filter(runOk).length >= 2, round, maxRounds: rounds, sources: runs.map(toStat), answers: runs.map((run) => ({ role: run.role, model: run.model, text: runOk(run) ? run.text : `FAILED: ${runError(run)}`, slotId: run.slot!.id, slotName: run.slot!.name, color: run.slot!.color, primary: run.slot!.primary })), artifactsDir }, runs.map((run) => `## [${run.slot!.name.toUpperCase()}]\n${runOk(run) ? run.text : `FAILED: ${runError(run)}`}`).join("\n\n"));
					}
				}

				const survivors = runs.filter(runOk);
				const ok = survivors.length >= 2;
				h.panel({ kind: "closing", command: "titan-debate", ok, round: rounds, maxRounds: rounds, prompt, sources: runs.map(toStat), answers: runs.map((run) => ({ role: run.role, model: run.model, text: runOk(run) ? run.text : `FAILED: ${runError(run)}`, slotId: run.slot!.id, slotName: run.slot!.name, color: run.slot!.color, primary: run.slot!.primary })), artifactsDir, ...h.totals(runs, startedAt) }, runs.map((run) => `## [${run.slot!.name.toUpperCase()}] FINAL OPINION\n${runOk(run) ? run.text : `FAILED: ${runError(run)}`}`).join("\n\n"));
				await h.save(artifactsDir, "summary.json", JSON.stringify({ command: "titan-debate", ok, rounds, survivors: survivors.map((run) => run.slot!.id), agents: runs.map(toStat), sessions: Object.fromEntries(slots.map((slot) => [slot.id, runs.find((run) => run.slot?.id === slot.id)?.sessionRef ?? h.cachedSlotId(slot)])), ...h.totals(runs, startedAt) }, null, 2));
			} finally {
				await h.ensureSummary(artifactsDir, { command: "titan-debate", ok: false, stopped: stopper.stopped(), rounds, agents: runs.map(toStat), sessions: Object.fromEntries(slots.map((slot) => [slot.id, runs.find((run) => run.slot?.id === slot.id)?.sessionRef ?? h.cachedSlotId(slot)])), ...h.totals(runs, startedAt) });
				stopper.release();
				stopWidget();
				ctx.ui.setStatus(CUSTOM_TYPE, undefined);
			}
