...
| `mcp-cli-bridges` | bridge | Call any MCP server from bash: titan's own `node scripts/mcp2cli.mjs list\|tools\|call\|doctor` (0.9.0, the shipped stdio bridge over the merged catalog), the Python mcp2cli (knowsuchagency, `uvx`) or mcporter (TypeScript); named links per server; not the Rust mcp2cli the Grok plugin uses |
