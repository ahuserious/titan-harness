...
description: Combo workflow: Figma design → tokens → shadcn/Relume components → Framer or app code, with the titan-harness fanning out builders. Use when a task goes from a design file to shipped UI.
---
...
3. **Plan** (`/titan-opinion` or `/titan-collaborate` in titan-harness): architect proposes the component map: which sections come from Relume (`relume-library`), which primitives from shadcn (`shadcn-components`), which are custom.
